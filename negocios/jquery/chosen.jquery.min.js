...
(function(){
    ...
    SelectParser=function(){
        function SelectParser(){
            ...
            return child.nodeName==="OPTGROUP"?this.add_group(child):this.add_option(child)
            ...
            if(option.nodeName==="OPTION")return option.text!==""?(group_position!=null&&(this.parsed[group_position].children+=1),this.parsed.push({
    ...
    root=this,$=jQuery,$.fn.extend({
        chosen:function(options){
            return!$.browser.msie||$.browser.version!=="6.0"&&$.browser.version!=="7.0"?this.each(function(input_field){
                var $this;
                $this=$(this);
    ...
    return this.is_multiple&&this.max_selected_options<=this.choices?(this.form_field_jq.trigger("liszt:maxselected",{
        chosen:this
    }),!1):(choice_id=this.container_id+"_c_"+item.array_index,this.choices+=1,this.search_container.before('<li class="search-choice" id="'+choice_id+'"><span>'+item.html+'</span><a href="javascript:void(0)" class="search-choice-close" rel="'+item.array_index+'"></a></li>'),link=$("#"+choice_id).find("a").first(),link.click(function(evt){
    ...
    return this.choices-=1,this.show_search_field_default(),this.is_multiple&&this.choices>0&&this.search_field.val().length<1&&this.results_hide(),this.result_deselect(link.attr("rel")),link.parents("li").first().remove()
    ...
    return result_data=this.results_data[pos],result_data.selected=!1,this.form_field.options[result_data.options_index].selected=!1,result=$("#"+this.container_id+"_o_"+pos),result.removeClass("result-selected").addClass("active-result").show(),this.result_clear_highlight(),this.winnow_results(),this.form_field_jq.trigger("change",{
